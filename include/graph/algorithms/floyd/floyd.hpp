...
class FloydWarshall {
 public:
  ...
  weight_type getDistance(const vertex_type &from,
  ...
  std::vector<vertex_type> retrievePath(const vertex_type &from,
                                        const vertex_type &to) const {
    if (negative_cycle_)
      throw std::runtime_error("График содержит отрицательный цикл");
  ...
  const std::unordered_map<vertex_type,
                           std::unordered_map<vertex_type, weight_type>> &
  getDistances() const {
    return distance_;
  }
 ...
 private:
  const Graph &graph_;
  std::unordered_map<vertex_type, std::unordered_map<vertex_type, weight_type>>
      distance_;
  std::unordered_map<vertex_type, std::unordered_map<vertex_type, vertex_type>>
      next_;
