...
public:
    ...
    using weight_type = decltype(std::declval<Graph>().getEdgeWeight(vertex_type(), vertex_type()));
    ...
    weight_type eccentricity(const vertex_type &vertex) const {
        ...
        for (const auto &pair: it->second) {
            if (pair.second != std::numeric_limits<weight_type>::max() && pair.second > max_dist) {
