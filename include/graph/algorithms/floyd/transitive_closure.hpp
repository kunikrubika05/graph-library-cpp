...
class TransitiveClosure {
 public:
  ...
  void compute() {
    ...
    for (const auto &k : vertices) {
      for (const auto &i : vertices) {
        for (const auto &j : vertices) {
          reachable_[i][j] =
              reachable_[i][j] || (reachable_[i][k] && reachable_[k][j]);
        }
      }
    }
  }
  ...
  const std::unordered_map<vertex_type, std::unordered_map<vertex_type, bool>> &
 ...
 private:
  const Graph &graph_;
  std::unordered_map<vertex_type, std::unordered_map<vertex_type, bool>>
      reachable_;
};
