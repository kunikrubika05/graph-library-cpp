...
public:
    ...
    using HeuristicFunction = std::function<weight_type(const vertex_type &, const vertex_type &)>;
    ...
    void compute(const vertex_type &source, const vertex_type &target) {
        ...
        std::priority_queue<queue_element, std::vector<queue_element>, decltype(compare)> openSet(compare);
        ...
        while (!openSet.empty()) {
            ...
            for (const auto &neighbor: graph_.getNeighbors(current)) {
                weight_type tentative_g = g_score_[current] + graph_.getEdgeWeight(current, neighbor);
    ...
    const std::unordered_map<vertex_type, weight_type> &getAllDistances() const { return g_score_; }
