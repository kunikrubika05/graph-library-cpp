...
namespace graph {
    ...
    class UndirectedAdjacencyMatrix : public GraphBase<UndirectedAdjacencyMatrix<VertexType, WeightType>, VertexType, WeightType> {
    public:
        using ErrorHandler = std::function<void(GraphErrorCode, const std::string &)>;

        UndirectedAdjacencyMatrix(std::size_t maxVertices,
            ...
            : GraphBase<UndirectedAdjacencyMatrix, VertexType, WeightType>(options),
              capacity_(maxVertices), count_(0), errorPolicy_(errorPolicy) {
        ...
        UndirectedAdjacencyMatrix &addEdge(const VertexType &v1, const VertexType &v2, const WeightType &weight) override {
        ...
        UndirectedAdjacencyMatrix &removeEdge(const VertexType &v1, const VertexType &v2) override {
            auto it1 = indices_.find(v1);
            auto it2 = indices_.find(v2);
            if (it1 == indices_.end() || it2 == indices_.end()) {
                handleError(GraphErrorCode::VERTEX_NOT_FOUND);
