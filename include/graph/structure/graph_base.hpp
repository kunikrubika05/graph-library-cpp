...
class GraphBase {
 public:
  explicit GraphBase(GraphOptions options = GraphOptions())
      : options_(options) {}
  ...
  virtual Derived &addEdge(const VertexType &from, const VertexType &to,
                           const WeightType &weight) = 0;
  ...
  virtual std::vector<VertexType> getNeighbors(
      const VertexType &vertex) const = 0;
  virtual WeightType getEdgeWeight(const VertexType &from,
                                   const VertexType &to) const = 0;
