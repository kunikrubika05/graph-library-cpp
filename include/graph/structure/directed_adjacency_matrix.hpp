...
namespace graph {
    ...
    class DirectedAdjacencyMatrix : public GraphBase<DirectedAdjacencyMatrix<VertexType, WeightType>, VertexType, WeightType> {
    public:
        ...
        DirectedAdjacencyMatrix &addEdge(const VertexType &from, const VertexType &to, const WeightType &weight) override {
        ...
        DirectedAdjacencyMatrix &removeEdge(const VertexType &from, const VertexType &to) override {
            auto fromIt = indices_.find(from);
            auto toIt = indices_.find(to);
            if (fromIt == indices_.end() || toIt == indices_.end()) {
                handleError(GraphErrorCode::VERTEX_NOT_FOUND);
