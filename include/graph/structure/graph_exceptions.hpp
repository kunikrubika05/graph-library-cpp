...
inline GraphException vertexAlreadyExists() {
  return GraphException(GraphErrorCode::VERTEX_ALREADY_EXISTS,
                        error_messages[GraphErrorCode::VERTEX_ALREADY_EXISTS]);
}

inline GraphException vertexNotFound() {
  return GraphException(GraphErrorCode::VERTEX_NOT_FOUND,
                        error_messages[GraphErrorCode::VERTEX_NOT_FOUND]);
}

inline GraphException edgeAlreadyExists() {
  return GraphException(GraphErrorCode::EDGE_ALREADY_EXISTS,
                        error_messages[GraphErrorCode::EDGE_ALREADY_EXISTS]);
}

inline GraphException edgeNotFound() {
  return GraphException(GraphErrorCode::EDGE_NOT_FOUND,
                        error_messages[GraphErrorCode::EDGE_NOT_FOUND]);
}

inline GraphException selfLoopsForbidden() {
  return GraphException(GraphErrorCode::SELF_LOOPS_FORBIDDEN,
                        error_messages[GraphErrorCode::SELF_LOOPS_FORBIDDEN]);
}

inline GraphException operationNotSupported() {
  return GraphException(
      GraphErrorCode::OPERATION_NOT_SUPPORTED,
      error_messages[GraphErrorCode::OPERATION_NOT_SUPPORTED]);
}
