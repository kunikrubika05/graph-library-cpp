...
TEST(UndirectedAdjacencyMatrixTest, NeighborsTest) {
  ...
  EXPECT_TRUE(std::find(neighbors.begin(), neighbors.end(), 2) !=
              neighbors.end());
  EXPECT_TRUE(std::find(neighbors.begin(), neighbors.end(), 3) !=
              neighbors.end());
}
...
TEST(UndirectedAdjacencyMatrixTest, EdgePerformanceTest) {
  ...
  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> elapsed = end - start;
  std::cout << "Time to add " << count - 1 << " edges: " << elapsed.count()
            << " sec\n";
