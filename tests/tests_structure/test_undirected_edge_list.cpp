...
TEST(UndirectedEdgeListTest, SelfLoopError) {
  ...
  UndirectedEdgeList<std::string, double> graph(options,
                                                ErrorPolicy::THROW_EXCEPTIONS);
  graph.addVertex("Alice");
...
TEST(UndirectedEdgeListTest, GetNeighborsTest) {
  UndirectedEdgeList<std::string, double> graph;
  graph.addVertex("Alice").addVertex("Bob").addVertex("Charlie").addVertex(
      "Dave");
  graph.addEdge("Alice", "Bob", 1.0);
  graph.addEdge("Alice", "Charlie", 1.1);
  auto neighbors = graph.getNeighbors("Alice");
  EXPECT_EQ(neighbors.size(), 2);
  EXPECT_TRUE(std::find(neighbors.begin(), neighbors.end(), "Bob") !=
              neighbors.end());
  EXPECT_TRUE(std::find(neighbors.begin(), neighbors.end(), "Charlie") !=
              neighbors.end());
}
