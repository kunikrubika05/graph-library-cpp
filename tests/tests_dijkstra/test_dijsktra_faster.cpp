...
TEST(OptimizedDijkstraTest, SimpleGraphWithTargetEarlyExit) {
    ...
    int target = 5;
    graph::algorithms::OptimizedDijkstra<graph::DirectedAdjacencyList<int, int>> dijkstra(graph);
...
TEST(OptimizedDijkstraTest, UnreachableVertex) {
    ...
    graph.addEdge(1, 2, 1);
    graph::algorithms::OptimizedDijkstra<graph::DirectedAdjacencyMatrix<int, int>> dijkstra(graph);
    dijkstra.compute(1);
