...
TEST(GraphCondensationTest, SimpleDirectedEdge) {
    ...
    EXPECT_EQ(condensation.getComponentId(1), condensation.getCondensedEdges()[0].first);
    EXPECT_EQ(condensation.getComponentId(2), condensation.getCondensedEdges()[0].second);
}
...
TEST(GraphCondensationTest, SimpleCycle) {
    ...
    EXPECT_EQ(condensation.getCondensedVertices().size(), 1);
    EXPECT_TRUE(componentsEqual(condensation.getCondensedVertices(), {{1, 2, 3}}));
    EXPECT_TRUE(condensation.getCondensedEdges().empty());
}
