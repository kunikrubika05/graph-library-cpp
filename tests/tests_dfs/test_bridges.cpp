...
bool containsEdge(const std::vector<std::pair<T, T>> &bridges, T v1, T v2) {
    return std::find_if(bridges.begin(), bridges.end(), [v1, v2](const auto &edge) {
               return (edge.first == v1 && edge.second == v2) || (edge.first == v2 && edge.second == v1);
           }) != bridges.end();
}
...
TEST(BridgesTest, TreeAllEdgesAreBridges) {
    ...
    graph.addEdge(1, 2, 1.0).addEdge(1, 3, 1.0).addEdge(3, 4, 1.0).addEdge(3, 5, 1.0);
